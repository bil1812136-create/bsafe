...
class _AnchorListTableState extends State<AnchorListTable> {
  ...
  void _saveEditing(int index, UwbAnchor anchor) {
    ...
    widget.onAnchorChanged?.call(
      index,
      UwbAnchor(
          id: anchor.id, x: newX, y: newY, z: newZ, isActive: anchor.isActive),
    );
    setState(() => _editingIndex = null);
  }

  void _cancelEditing() => setState(() => _editingIndex = null);
      ...
      child: Column(
        ...
        children: [
          ...
          ...widget.anchors.asMap().entries.map((entry) {
            return _buildAnchorRow(context, entry.key, entry.value);
          }),
        ],
      ),
    );
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: 50,
              child: Text(tag.x.toStringAsFixed(3),
                  style: const TextStyle(fontSize: 11),
                  textAlign: TextAlign.center),
            ),
            SizedBox(
              width: 50,
              child: Text(tag.y.toStringAsFixed(3),
                  style: const TextStyle(fontSize: 11),
                  textAlign: TextAlign.center),
            ),
            SizedBox(
              width: 50,
              child: Text(tag.z.toStringAsFixed(3),
                  style: const TextStyle(fontSize: 11),
                  textAlign: TextAlign.center),
            ),
            SizedBox(
              width: 50,
              child: Text(tag.r95.toStringAsFixed(3),
                  style: const TextStyle(fontSize: 11),
                  textAlign: TextAlign.center),
            ),
            for (int i = 0; i < anchors.length && i < 8; i++)
...
class UwbDataPanel extends StatefulWidget {
  ...
  const UwbDataPanel({super.key, required this.uwbService});
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              uwbService.addAnchor(
                  UwbAnchor(id: idController.text, x: x, y: y, z: z));
              Navigator.pop(context);
              ...
              child: Column(
                ...
                children: [
                  ...
                  AnchorListTable(
                    ...
                    onDeleteAnchor: (index) => uwbService.removeAnchor(index),
