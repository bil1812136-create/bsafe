...
class UwbSettingsPanel extends StatefulWidget {
  ...
  const UwbSettingsPanel({super.key, required this.uwbService, this.onClose});
...
class _UwbSettingsPanelState extends State<UwbSettingsPanel>
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCheckboxRow('顯示基站列表', config.showAnchorList,
              (v) => _updateConfig(config.copyWith(showAnchorList: v))),
          _buildCheckboxRow('自動獲取基站坐標', config.autoGetAnchorCoords,
              (v) => _updateConfig(config.copyWith(autoGetAnchorCoords: v))),
          _buildCheckboxRow('顯示標籤列表', config.showTagList,
              (v) => _updateConfig(config.copyWith(showTagList: v))),
          _buildCheckboxRow('顯示歷史軌跡', config.showHistoryTrajectory,
              (v) => _updateConfig(config.copyWith(showHistoryTrajectory: v))),
          _buildCheckboxRow('軌跡/導航模式', config.showTrajectory,
              (v) => _updateConfig(config.copyWith(showTrajectory: v))),
          _buildCheckboxRow('區域圍欄模式', config.showFence,
              (v) => _updateConfig(config.copyWith(showFence: v))),
          const SizedBox(height: 16),
          _buildSectionTitle('區域圍欄模式'),
          _buildNumberInputRow('區域1 (m)', _area1Controller,
          ...
          _buildDropdownRow('定位模式', config.positioningMode, ['二維定位', '三維定位'],
              (v) => _updateConfig(config.copyWith(positioningMode: v))),
          _buildDropdownRow(
              '定位算法',
              config.algorithm,
              ['卡爾曼/平均算法', '最小二乘法', '三邊定位'],
              (v) => _updateConfig(config.copyWith(algorithm: v))),
          const SizedBox(height: 16),
          ...
          _buildNumberInputRow('係數 a', _correctionAController,
              (v) => _updateConfig(config.copyWith(correctionA: v))),
          _buildNumberInputRow('係數 b', _correctionBController,
              (v) => _updateConfig(config.copyWith(correctionB: v))),
          const SizedBox(height: 8),
          SizedBox(
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () {
                _updateConfig(config.copyWith(distanceIndexMap: [0, 1, 2, 3]));
                setState(() {});
              },
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: Text(
                      widget.uwbService.isLoadingFloorPlan ? '載入中...' : '打開'),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
          ...
          _buildNumberInputRowWithUnit('X 偏移', _xOffsetController, '米',
              (v) => _updateConfig(config.copyWith(xOffset: v))),
          _buildNumberInputRowWithUnit('Y 偏移', _yOffsetController, '米',
              (v) => _updateConfig(config.copyWith(yOffset: v))),
          const SizedBox(height: 16),
          _buildSectionTitle('比例設置'),
          _buildNumberInputRowWithUnit('X 比例', _xScaleController, '像素/米',
              (v) => _updateConfig(config.copyWith(xScale: v))),
          _buildNumberInputRowWithUnit('Y 比例', _yScaleController, '像素/米',
              (v) => _updateConfig(config.copyWith(yScale: v))),
          const SizedBox(height: 16),
          _buildSectionTitle('翻轉設置'),
          _buildCheckboxRow('翻轉 X', config.flipX,
              (v) => _updateConfig(config.copyWith(flipX: v))),
          _buildCheckboxRow('翻轉 Y', config.flipY,
              (v) => _updateConfig(config.copyWith(flipY: v))),
          _buildCheckboxRow('顯示原點', config.showOrigin,
              (v) => _updateConfig(config.copyWith(showOrigin: v))),
          if (config.floorPlanImagePath != null) ...[
            const SizedBox(height: 8),
            Container(
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('• 圖片：PNG, JPG, BMP, GIF, WEBP',
                    style:
                        TextStyle(fontSize: 11, color: Colors.blue.shade800)),
                const SizedBox(height: 2),
                Text('• 向量圖：SVG（可調整大小不失真）',
                    style:
                        TextStyle(fontSize: 11, color: Colors.blue.shade800)),
                const SizedBox(height: 2),
                Text('• 文件：PDF（自動擷取第一頁）',
                    style:
                        TextStyle(fontSize: 11, color: Colors.blue.shade800)),
                const SizedBox(height: 2),
                Text('• 工程圖：DWG/DXF（請先轉換為 PDF 或 SVG）',
                    style:
                        TextStyle(fontSize: 11, color: Colors.blue.shade800)),
                const SizedBox(height: 6),
                Text('提示：X/Y比例 = 圖片上每米對應的像素數',
                    style:
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildNumberInputRowWithUnit('寬度', _gridWidthController, '米',
              (v) => _updateConfig(config.copyWith(gridWidth: v))),
          _buildNumberInputRowWithUnit('高度', _gridHeightController, '米',
              (v) => _updateConfig(config.copyWith(gridHeight: v))),
          const SizedBox(height: 16),
          _buildCheckboxRow('顯示網格', config.showGrid,
              (v) => _updateConfig(config.copyWith(showGrid: v))),
  ...
  Widget _buildQuickGridButton(String label, double size) {
    return OutlinedButton(
      onPressed: () {
        ...
        _updateConfig(widget.uwbService.config
            .copyWith(gridWidth: size, gridHeight: size));
      },
      child: Text(label),
    );
  }
        ...
        children: [
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => Navigator.pop(context, 'search_ports'),
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('• BU04 設備使用 CH340 或 CP210x 芯片',
                    style:
  ...
  String _describeMapping(List<int> map) {
    ...
    final swaps = <String>[];
    for (int i = 0; i < 4; i++) {
      if (map[i] != i) swaps.add('硬體D$i → 基站${map[i]}');
    }
    return swaps.join('，');
  }
  ...
  Widget _buildSwapButton(String label, List<int> mapping) {
    ...
    return SizedBox(
      ...
      child: isActive
          ...
          : OutlinedButton(
              onPressed: () {
                _updateConfig(config.copyWith(distanceIndexMap: mapping));
                setState(() {});
              },
