...
class SeveritySelector extends StatelessWidget {
  ...
  const SeveritySelector(
      {super.key, required this.selectedSeverity, required this.onSelected});
  ...
  IconData _getSeverityIcon(String s) {
    switch (s) {
      case 'severe':
      ...
      children: ReportModel.severities.map((severity) {
        ...
        return Expanded(
          child: GestureDetector(
            ...
            child: Container(
              margin:
                  EdgeInsets.only(right: severity['value'] != 'severe' ? 8 : 0),
              padding: const EdgeInsets.symmetric(vertical: 14),
              decoration: BoxDecoration(
                color: isSelected ? color : Colors.white,
                borderRadius: BorderRadius.circular(10),
                border: Border.all(
                    color: isSelected ? color : AppTheme.borderColor, width: 2),
